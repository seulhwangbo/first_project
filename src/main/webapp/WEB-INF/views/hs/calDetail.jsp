				...
				<td colspan="2" style="text-align: right;">
					...
					<c:if test="${calTotal.eventCategory != 100 }">
						<c:if test="${calTotal.attendRsvp != 110 }">
							<input type="button" class="btn btn-outline-success btn-sm" value="수정" onclick="location.href='/hs/calUpdate?eventId=${calTotal.eventId}'">
							<form action="deleteEvent" method="post" style="display: inline;">
								...
								<button type="submit" class="btn btn-outline-danger btn-sm" onclick="confirmDel(${calTotal.eventId})">삭제</button>
							</form>
						</c:if>
					</c:if>
				</td>
