...
<main>
	...
	<c:if test="${page.startPage > page.pageBlock}">
		<a href="appFin?currentPage=${page.startPage-page.pageBlock}">[이전]</a>
	</c:if>
	<c:forEach var="i" begin="${page.startPage}" end="${page.endPage }">
		<a href="appFin?currentPage=${i}">[${i }]</a>
	</c:forEach>
	<c:if test="${page.endPage < page.totalPage}">
		<a href="appFin?currentPage=${page.startPage + page.pageBlock}">[다음]</a>
	</c:if>	
	
	<p><a href="draftingForm">기안 작성</a><p>
	
	<form action="finAppListSearch">
		<select name="search">
